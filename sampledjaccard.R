...
jaccard_intersection <- function(x1, y1, r1, x2, y2, r2){
  ...
  # x2 <- plist[[3*id2]]$x
  # y2 <- plist[[3*id2]]$y
  # r1 <- 1 + r*plist[[3*id1]]$s
  # r2 <- 1 + r*plist[[3*id2]]$s
...
for (i in index_list) {
  random_ids <- sample.int(nrow(i), ceiling(nrow(i)/11))
  indexplays <- i[random_ids, 2]
  indexgames <- i[random_ids, 1]
  testsample <- og_sample[og_sample$gameId %in% indexgames, ]
  testsample <- testsample[testsample$playId %in% indexplays, ]
