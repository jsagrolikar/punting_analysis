...
jaccard <- function(id1, id2, plist) {
  ...
  for (i in 1:10) {
    ...
    + sum(apply(pts2, 1, function(z) (circle_intersection(as.numeric(z["x"]), as.numeric(z["y"]), 1+r*abs(as.numeric(z["x"])), px2, py2, 1+r*abs(px2)))))
  }
  return(j_acc)
}
...
mdmat <- function(p, plist) {
  ...
  # dist_mat <- outer(1:p, 1:p, Vectorize(function(x, y) jaccard(x, y, plist)))
  for (i in 1:(p-1)) {
    dist_mat[i, ] <- c(1:p)
    vec <- sapply(dist_mat[i, (i+1):p], Vectorize(function(x) jaccard(i, x, plist)))
...
for (i in 1:n) {
  ...
  spec_punt_df <- subset(spec_punt_df, select=-c(X, dis, o, dir, displayName, jerseyNumber, position, gameDate, gameTimeEastern))
...
while (n>=2) {
  ...
  preserved_cluster <- min(mincol, minrow)
  unpreserved_cluster <- max(mincol, minrow)
